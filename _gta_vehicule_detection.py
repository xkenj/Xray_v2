...
def determine_movement(mid_x, mid_y, width=800, height=640):
    '''
    - center of the screen = 0.5, 0.5 / center of the object = mid_x, mid_y
    '''
    x_move = 0.5 - mid_x # distance from the center
    y_move = 0.5 - mid_y
    hm_x = x_move / 0.5  # how much to move
    hm_y = y_move / 0.5

    keys.keys_worker.SendInput(keys.keys_worker.Mouse(0x0001, -1*int(hm_x*width), -1*int(hm_y*height))) # Go to that object
    ...
    while True:                                        #800,640
        ...
        for i, b in enumerate(boxes[0]):
            ...
            if classes[0][i] == 3 or classes[0][i] == 6 or classes[0][i] == 8: # compute the number of pixels between x1 and x2 
                if scores[0][i] > 0.5:  # As long as the score is above 50% (confident it is a car)
                    ...
                    stolen_distance = round( (1 - (boxes[0][i][3] - boxes[0][i][1]))**4, 3) # For car stolen, Don't flag too much
                    vehicule_dict[stolen_distance] = [mid_x, mid_y, scores[0][i]]
                    cv2.putText(image_np, '{} meters !'.format(
                        warning_distance*10), (int(mid_x * 800), int(mid_y*450)), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

                    # WARNING:
                      
                    if warning_distance <= 0.5:
